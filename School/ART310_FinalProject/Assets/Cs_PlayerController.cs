...
public class Cs_PlayerController : MonoBehaviour
{
    ...
    void PlayerMovement(Vector3 v3_Direction_, bool b_Jump_, float f_Magnitude_ = 1)
    {
        ...
        Vector3 v3_newVelocity = Vector3.Lerp(v3_oldVelocity, v3_FinalRotation * f_MaxMoveSpeed_Curr * f_Magnitude_, 1 / ACCELERATION);
    ...
    float f_MaxMoveSpeed_Curr;
    float f_FOV_Speed = 150f;
    void Sprint()
    {
        bool b_Moving = false;
        if (this_Rigidbody.velocity.magnitude > 0.01f) b_Moving = true; else b_Moving = false;

        if(Input.GetKey(KeyCode.LeftShift) || Input.GetKey(KeyCode.LeftControl))
        {
            // Increase move speed
            f_MaxMoveSpeed_Curr = MAX_MOVESPEED_FORWARD * 1.75f;

            // Increase FOV
            float f_FOV = this_Camera.GetComponent<Camera>().fieldOfView;
            if( f_FOV < 75f && b_Moving )
            {
                f_FOV += Time.fixedDeltaTime * f_FOV_Speed;

                if ( f_FOV > 75f ) f_FOV = 75f;
        ...
        else
        {
            ...
            // Reset Move Speed
            f_MaxMoveSpeed_Curr = MAX_MOVESPEED_FORWARD;
            ...
            if (f_FOV > 60f )
            {
                ...
                if (f_FOV < 60f && b_Moving ) f_FOV = 60f;
