...
namespace ProBuilder2.EditorCommon
{
	...
	public class pb_BooleanInterface : EditorWindow
	{
		...
		{	
			...
			switch(e.type)
			{
				...
				case EventType.MouseUp:
					if(mouseClickedSwapRect && swapOrderRect.Contains(UnityEngine.Event.current.mousePosition))
					{
		...
		{
			Vector2 mPos = UnityEngine.Event.current.mousePosition;
			...
			if( (UnityEngine.Event.current.type == EventType.DragUpdated || UnityEngine.Event.current.type == EventType.DragPerform) && DragAndDrop.objectReferences.Length > 0)
			{
